...
async def get_travel_time(
    ...
    # The Routes API REQUIRES a FieldMask header to know what data to return
    headers = {
    ...
    # The new JSON payload structure
    payload = {
        "origin": {
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            # If Google rejects it, pass the error directly to the console
            ...
            if "routes" in data and len(data["routes"]) > 0:
                ...
                # The new API returns duration as a string like "900s". We need to parse it.
                duration_str = route.get("duration", "0s")
                duration_seconds = int(duration_str.replace("s", ""))
                distance_meters = route.get("distanceMeters", 0)

                # Format text like the old API so your Android app doesn't break
